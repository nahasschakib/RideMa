...
export async function POST(
  ...
  try {
    ...
    if (booking.bookingStatus !== "completed") {
      return NextResponse.json({ error: "La course n'est pas terminée" }, { status: 400 });
    }
    if (booking.rating?.score !== null && booking.rating?.score !== undefined && booking.rating?.score > 0) {
      return NextResponse.json({ error: "Course déjà notée" }, { status: 400 });
    }
    booking.rating = { score, comment: comment || "", ratedAt: new Date() };
    await booking.save();
    const driver = await User.findById(booking.driver);
    if (driver) {
      const total = (driver.totalRatings || 0) + 1;
      const avg = ((driver.averageRating || 0) * (total - 1) + score) / total;
