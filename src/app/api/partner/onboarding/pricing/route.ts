    ...
    try {
        await dbConnect();
            ...
            if (!session || !session.user?.email) {
              return Response.json({ message: "partenaire non autorisé" }, { status: 401 });
            }
        
            const partner = await User.findOne({ email: session.user.email });
            if (!partner) {
              return Response.json({ message: "partenaire non trouvable" }, { status: 404 });
            }
