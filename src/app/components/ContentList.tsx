...
type ContentType = "partner" | "kyc" | "vehicle";
...
interface ContentItem {
  ...
  owner?: {        // ✅ optionnel — objet avec name et email
    name: string;
    email: string;
  };
}
...
const handleStartVideoKyc= async(id:unknown)=>{
  try {
...
border-gray-200 shadow-sm"
      >
        ...
        <p className="font-bold text-gray-800 text-base">Tous retenus !</p>
        <p className="text-sm text-gray-400 mt-1">
          Aucune attente pour le moment
        </p>
      </motion.div>
    );
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between px-1 mb-1">
        <p className="text-xs font-semibold uppercase tracking-widest text-gray-400">
          {type === "partner"
            ? "File d'attente des avis des partenaires"
            : type === "kyc"
              ? "File d'attente vidéo"
              : "File d'attente des avis sur les véhicules"}
        </p>
        ...
        return (
          <motion.div
            ...
            className="bg-white border border-gray-100 rounded-2xl px-5 py-4 
                flex items-center justify-between gap-4 shadow-sm transition-shadow"
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              >
                {name.charAt(0).toUpperCase() ?? <Users size={14} />}
              </div>
